...
$ti = dle_cache("last_time_news2", $member_id['user_id']);
if($ti!="0"&&$ti)
{
	 create_cache("last_time_news2", "0", $member_id['user_id']);
	 }
	 	$ti = dle_cache("last_time_news3", $member_id['user_id']);
if($ti!="0"&&$ti)
{
	 create_cache("last_time_news3", "0", $member_id['user_id']);
}
	...
	{
		$tpl->load_template( 'nightvideo.tpl' );
	}
	else{
		$tpl->load_template( 'shortstory2.tpl' );
	}
	}
	
	elseif( $category_id and $cat_info[$category_id]['short_tpl'] != '' ) $tpl->load_template( $cat_info[$category_id]['short_tpl'] . '.tpl' );
    else $tpl->load_template( 'nightnew.tpl' );
	
	 $ava = $db->get_row($db->query("SELECT foto FROM ".PREFIX."_users WHERE name='".$row['autor']."'"));
    if($ava['foto']) $tpl->set('{poster_avatar}', "<img hspace=\"2\" vspace=\"2\" align=\"left\" src=\"".$config['http_home_url']."uploads/fotos/".$ava['foto']."\">");
    else $tpl->set('{poster_avatar}', "<img hspace=\"2\" vspace=\"2\" align=\"left\" src=\"/templates/Default/images/noavatar.png\">");
		...
		if( $config['allow_tags'] and $row['tags'] ) {
			...
			foreach ( $row['tags'] as $value ) {
				...
				$go_page = ($config['ajax']) ? "onclick=\"DlePage('do=tags&amp;!tag=" .$value . "'); return false;\" " : "";
		...
		if( $row['allow_rate'] ) {
			...
			if( $config['short_rating'] and $user_group[$member_id['user_group']]['allow_rating'] ) $tpl->set( '{rating}', ShortRating2( $row['id'], $row['rating'], $row['vote_num'], 1 ) );
			else $tpl->set( '{rating}', ShortRating2( $row['id'], $row['rating'], $row['vote_num'], 0 ) );
		
		} else
			$tpl->set( '{rating}', "" );
		
			$tpl->set( '{ratingshort}',  $row['rating']);
			
					$tpl->set( '{game2}', $row['type'] );
		if( $config['ajax'] ) {
		...
		$tpl->set('{usertitle}', stripslashes($row['autor']));
		
				$tpl->set( '[com-link2]', "<a {$go_page}href=\"" . $full_link . "\">" );
			$tpl->set( '[/com-link2]', "</a>" );
		if( $row['allow_comm'] ) {
		...
		if( $allow_userinfo and ! $row['approve'] ) {
			...
			if($member_id[skin]==2||$_SESSION[skin]==2)
{
		$tpl->set( '{compic}', "commdark" );
	$tpl->set( '{eyepic}', "eyedark" );
}
else
{
	$tpl->set( '{compic}', "comments" );
	$tpl->set( '{eyepic}', "eyewhite" );
}
 ...
 $pages = @ceil($row['comm_num']/intval($config['comm_nummers']));
    ...
    if($p == "") $p = "<a href='$PHP_SELF?newsid=$row[id]#comment'>+</a>&nbsp;";
