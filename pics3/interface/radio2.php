	...
	$pot = $db->super_query("SELECT *,  WEEKDAY(begin) as beg, end as minutes FROM radioadmin where id='{$idp}' order by id desc LIMIT 0,1;");
	 ...
	 $when=date("H:i", $time1);
	  ...
	  $nam_e =stripslashes($pot[name]);
				   ...
				   if($pot[link1])
				   {
				 $link1= '<a href="'.$pot[link1].'"><b>Архив выпусков передачи &raquo;</b></a><br>';
				   }
				    $tpl->set( '{potok7}', $link1);
				if($pot[link2])
				   {
				 $link2= '<a href="'.$pot[link2].'"><b>Тема для обсуждения передачи &raquo;</b></a><br>';
				   }
				   if($member_id[user_group]<4)
				   {
				 $link2.= '<a href="http://nocens.ru/index.php?do=radio2&djbar=1&prid='.$pot[id].'"><b>Редактировать в DjBAR &raquo;</b></a><br>';
				   }
				       $tpl->set( '{potok7}', $link1);
				if($pot[call]&&$pot['efir']=="1")
				   {
				 $link10= '<p style="font-size: 16px; color: #BCE7FE; font-style: italic;"><b>Прямой эфир, принимаются Skype-звонки!</b></p>';
				   }
				   else if ($pot['efir']=="1")
				   {
...
$pot = $db->super_query("SELECT *,  WEEKDAY(begin) as beg, end as minutes FROM radioadmin where wave='1' AND  ((WEEKDAY(begin)='".$wday."'  AND TIME(begin)<='".$hday."'  AND TIME(end)>='".$hday."') OR ( (WEEKDAY(begin)='".$wday."' AND WEEKDAY(end)='".$wdaynext."'  AND TIME(begin)<='".$hday."'  )) OR  (WEEKDAY(end)='".$wday."'  AND WEEKDAY(begin)='".$wdayprev."'  AND TIME(end)>='".$hday."')) order by type asc, id desc LIMIT 0,1;");
		 ...
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, TIME(begin) as h1, TIME(end) as h2 FROM radioadmin where wave='1' AND   WEEKDAY(begin)='".$wday."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
...
else if($_GET['todayxml'])
{
	   ...
	   if($wday=='-1')
	   $wday=6;
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where  wave='1' AND  WEEKDAY(begin)='".$wday."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if(isset($_GET['dayxml']))
{
		 ...
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where wave='1' AND   WEEKDAY(begin)='".$_GET['dayxml']."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if($_GET['weekxml'])
{
	 		 ...
	 		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where wave='1' AND  type='2' order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if($_GET['djbar']&&$member_id[user_group]<=3)
{
	...
	else if($_POST[potok]||$_POST[potok2])
	{
		...
		$typ=1;
			  if($_GET[prid])
			  {
		   $db->query( "UPDATE radioadmin set `src`='" . $potok . "', `src2`='" . $potok64 . "',`src3`='" . $potok256 . "', `efir`='".$efir."', `name`='" . $potok2 . "', `end`='".$month."', `begin`='".$month2."', `type`='".$typ."', `picture`='".$picl."',`link1`='".$link1."',`link2`='".$link2."',`description`='".$dsc."', `call`='".$call."',`dj`='".$djay."',`overlay`='".$overlay."'   where id = '".$_GET[prid]."'" );
			  }
			  else
			  {
		$db->query("DELETE FROM radioadmin WHERE type='1' && begin <= NOW()");
	
								  $db->query("INSERT  INTO radioadmin (`src`,`src2`,`src3`, `name`, `end`, `begin`, `type`, `picture`,`link1`,`link2`,`description`,`call`,`efir`,`dj`,`overlay`) values ('" . $potok . "','" . $potok64 . "','" . $potok256 . "','" . $potok2 . "', '".$month."', '".$month2."','".$typ."','".$picl."','".$link1."','".$link2."','".$dsc."','".$call."','".$efir."','".$djay."','".$overlay."')");
			  }
	 ...
	 $k1=0;
	    while ($im = $db->get_row($fsch)){
			if($im[beg]!=$weekd)
			{
				...
				if($im[src]!="")
				{
					$alert='<img title="Поток изменен на'.$im[src].'" src="http://nocens.ru/pics3/interface/lightning.png">';
				}
			$delb='<a href="http://nocens.ru/index.php?do=radio2&djbar=1&clear='.$im[id].'">[X]</a>';
		$progs.=$alert.$delb.$pl."[".substr($im[h1],0,strlen($im[h1])-3)."] <a href=\"http://nocens.ru/index.php?do=radio2&djbar=1&prid=".$im[id]."\">".$im[name].$pm."</a><br>";
