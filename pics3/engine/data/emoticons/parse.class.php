...
class ParseFilter {
	...
	var $blank_links=false;
var $tagBlacklist = array('applet', 'body', 'bgsound', 'base', 'basefont', 'frame', 'frameset', 'head', 'html', 'id', 'ilayer', 'layer', 'link', 'meta', 'name', 'style', 'title', 'xml','input','div');
	...
	function BB_Parse($source, $use_html = TRUE) {
		...
		if( $use_html == false ) {
			...
			$smilies2=",aloner,ayy,bleat,cuttie,donbass,fyeah,great,imaginer,kidding,maddy,mytongue,pokerface,seenit,shhhh,sicking,sthwrong,thinking,trollface,veryhappy,whyy,wowow,fffuuu,mkay,sondisappoint,sergey,no-meme,justin,raxemus,diarts,okeywink,darts,doctorStuha,fox,tiger,sleepy,torch,fastfood,paws,rainbow,gachiGASM,luck,goldenmic,megusta,sir,please,noway,kittie,lal,lalrainbow,ohMy,broagree,broangry,brocrazy,broglamour,brohug,browtf,brohappy,broidkn,brony,broretro,brosad,broskype,brotrololo,brocritic,broeww,brodj,broscared,brospike,bromustache,brocrystal,brosun,hugbear,nosloth,genius,kek,shame,slyderman,tearsflow,pepeSad,spainshame,ankorRAGE,OwO,wut";
			
		$smilies_arr = explode( ",", $config['smilies'].$smilies2 );
		foreach ( $smilies_arr as $smile ) {
			$smile = trim( $smile );
			$find[] = "':$smile:'";
			$replace[] = "<!--smile:{$smile}--><img style=\"vertical-align: middle;border: none;\" alt=\":$smile:\" title=\":$smile:\"  src=\"" . $config['http_home_url'] . "engine/data/emoticons/{$smile}.gif\" /><!--/smile-->";
		}
		
		$source = preg_replace( $find, $replace, $source );
		if(strpos($source, 'vk.com')===false&&strpos($source, 'collegehumor.com')===false&&strpos($source, 'yandex.ru')===false&&strpos($source, 'ustream.tv')===false&&strpos($source, 'youtu')===false&&strpos($source, 'twitch')===false&&strpos($source, 'livestream')===false&&strpos($source, 'coub')===false&&strpos($source, 'rutube.ru')===false)
		{
	$source = preg_replace( "#<iframe#i", "&lt;iframe", $source );
		...
		$source = preg_replace( "#\[b\](.+?)\[/b\]#is", "<b>\\1</b>", $source );
			...
			$source = preg_replace( "#\[user=(.+?)\]#is", "<a href=\"//nocens.net/index.php?subaction=userinfo&user=\\1\"><b>\\1</b></a>", $source );
			$source = preg_replace( "#\[blog=(.+?)\]#is", "<a href=\"//nocens.net/index.php?do=static&page=blogs&user=\\1\"><b>Блог \\1</b></a>", $source );
	...
	function decodeBBCodes($txt, $use_html = TRUE, $wysiwig = "no") {
		...
		$txt = preg_replace( "#<!--dle_audio_begin:(.+?)-->(.+?)<!--dle_audio_end-->#is", '[audio=\\1]', $txt );
			$txt = preg_replace( "#<a href=\"http://nocens.net/index.php?subaction=userinfo&user=.+?\">.+?</a>#", '[user=\\1]', $txt );
						$txt = preg_replace( "#<a href=\"https://nocens.net/index.php?subaction=userinfo&user=.+?\">.+?</a>#", '[user=\\1]', $txt );
	...
	$smilies2=",aloner,ayy,bleat,cuttie,donbass,fyeah,great,imaginer,kidding,maddy,mytongue,pokerface,seenit,shhhh,sicking,sthwrong,thinking,trollface,veryhappy,whyy,wowow,fffuuu,mkay,sondisappoint,sergey,justin,raxemus,diarts,okeywink,darts,doctorStuha,fox,tiger,sleepy,fastfood,paws,rainbow,luck,gachiGASM,torch,goldenmic,megusta,sir,please,noway,hugbear,nosloth,genius,kek,shame,slyderman,tearsflow,lal,lalrainbow,ohMy,pepeSad,spainshame,ankorRAGE,OwO,wut";
		$smilies_arr = explode( ",", $config['smilies'].$smilies2 );
	...
	function build_youtube($url) {
		...
		if ($source['host'] != "youtube.com"  AND $source['host'] != "twitch.tv" AND $source['host'] != "coub.com" ) return "[youtube=".$url."]";
		...
		if($source['host']=='twitch.tv'||$source['host']=='coub.com')
		{
			$video_link =  $url;
		}

		if ($source['host'] == "youtube.com")
			//return '<!--dle_youtube_begin:'.$url.'--><object width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'"><param name="movie" value="//www.youtube.com/v/'.$video_link.'&hl=ru&fs=1"></param><param name="wmode" value="transparent" /><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'.$video_link.'&hl=ru&fs=1" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" wmode="transparent" width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'"></embed></object><!--dle_youtube_end-->';
		return '<!--dle_youtube_begin:'.$url.'--><iframe width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'" src="//www.youtube.com/embed/'.$video_link.'" frameborder="0" allowfullscreen></iframe><!--dle_youtube_end-->';
		
		else if($source['host'] == "twitch.tv")
		{
			
			if(strpos($url,'/v/')!==false)
			{
				$masurl=explode('/', $url);
				return '<object bgcolor="#000000" data="//www.twitch.tv/swflibs/TwitchPlayer.swf" height="378" id="clip_embed_player_flash" type="application/x-shockwave-flash"  width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'"><param name="movie" value="//www.twitch.tv/swflibs/TwitchPlayer.swf" /><param name="allowScriptAccess" value="always" /><param name="allowNetworking" value="all" /><param name="allowFullScreen" value="true" /><param name="flashvars" value="channel='.$masurl[3].'&amp;auto_play=false&amp;start_volume=25&amp;videoId=v'.$masurl[5].'" /></object>';
			}
			else
			return '<!--dle_youtube_begin:'.$url.'--><iframe width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'" src="'.$video_link.'/embed" frameborder="0" allowfullscreen></iframe><!--dle_youtube_end-->';
			
		}
		
			else if($source['host'] == "coub.com")
		{
			
			return '<!--dle_youtube_begin:'.$url.'--><iframe width="'.$this->video_config['width'].'" height="'.$this->video_config['height'].'" src="'.$video_link.'?muted=false&autostart=false&originalSize=true&startWithHD=true" frameborder="0" allowfullscreen></iframe><!--dle_youtube_end-->';
	...
	function build_url($url = array()) {
		...
		if( ! preg_match( "#^(http|news|https|ed2k|ftp|aim|mms)://|(magnet:?)#", $url['html'] )&&! preg_match( "#^//|(magnet:?)#", $url['html'] ) ) {
			$url['html'] = '//' . $url['html'];
		}
		...
		if( $this->check_home( $url['html'] ) && $this->blank_links==false) $target = "";
...
//		$txt = stripslashes( $txt );
	...
	function build_video($url) {
		...
		if( $type == "flv" or $type == "mp4" or $type == "m4v" or $type == "m4a" or $type == "mov" or $type == "3gp" or $type == "f4v") {
			...
			return "<!--dle_video_begin:{$decode_url}--><object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" id=\"Player-{$id_player}\">
				...
				<embed src=\"" . $config['http_home_url'] . "engine/classes/flashplayer/media_player.swf?MediaLink={$url}&defaultMedia=1{$option[1]}{$watermark}&showPlayButton=true&playOnStart={$this->video_config['play']}{$color['backgroundBarColor']}{$color['btnsColor']}&outlineColor=0x666666{$color['outputBkgColor']}{$color['outputTxtColor']}{$color['loadingBarColor']}{$color['loadingBackgroundColor']}{$color['progressBarColor']}{$color['volumeBackgroundColor']}{$color['volumeStatusBarColor']}\" quality=\"high\" bgcolor=\"#000000\" wmode=\"opaque\" allowFullScreen=\"true\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" align=\"middle\" type=\"application/x-shockwave-flash\" pluginspage=\"//www.macromedia.com/go/getflashplayer\"></embed>
				</object><!--dle_video_end-->";

			
		} elseif( $type == "avi" or $type == "divx" ) {
			
			return "<!--dle_video_begin:{$decode_url}--><object classid=\"clsid:67DABFBF-D0AB-41fa-9C46-CC0F21721616\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" codebase=\"//go.divx.com/plugin/DivXBrowserPlugin.cab\">
				...
				<embed type=\"video/divx\" src=\"{$url}\" custommode=\"none\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" mode=\"zero\"  autoPlay=\"{$this->video_config['play']}\" previewImage=\"{$option[1]}\" pluginspage=\"//go.divx.com/plugin/download/\">
				</embed>
				</object><!--dle_video_end-->";
			...
			return "<!--dle_video_begin:{$url}--><object id=\"mediaPlayer\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" classid=\"CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6\" standby=\"Loading Microsoft Windows Media Player components...\" type=\"application/x-oleobject\">
				...
				<embed pluginspage=\"//www.microsoft.com/Windows/Downloads/Contents/MediaPlayer/\" src=\"{$url}\" width=\"{$this->video_config['width']}\" height=\"{$this->video_config['height']}\" type=\"application/x-mplayer2\" autorewind=\"1\" showstatusbar=\"1\" showcontrols=\"1\" autostart=\"{$this->video_config['play']}\" allowchangedisplaysize=\"1\" volume=\"70\" stretchtofit=\"1\"></embed>
				</object><!--dle_video_end-->";
		}
	...
	function build_audio($url) {
		...
		$width = $this->video_config['width'];
			$height = 27;
	return "<!--dle_audio_begin:{$list}--><object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0\" width=\"{$width}\" height=\"{$height}\" id=\"Player-{$id_player}\">
				...
				<param name=\"bgcolor\" value=\"#000000\" />
				<param name=\"wmode\" value=\"opaque\" />
				<embed src=\"" . $config['http_home_url'] . "engine/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=audio&amp;videoUrl={$url}&amp;showWatermark=false&amp;showPreviewImage=true&amp;previewImageUrl=&amp;autoPlays={$this->video_config['play']}&amp;isYouTube=false&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView=2&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=false&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=false&amp;autoHideNav=false&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" quality=\"high\" bgcolor=\"#000000\" wmode=\"opaque\" allowFullScreen=\"false\" width=\"{$width}\" height=\"{$height}\" align=\"middle\" type=\"application/x-shockwave-flash\" pluginspage=\"//www.macromedia.com/go/getflashplayer\"></embed>
