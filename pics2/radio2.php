...
$nexttime = time() + 30;
  ...
  $wdayprev=6;
	...
	$sname= ltrim(rtrim(str_replace ($vowels, " ",$_GET['sname'])));
	 ...
	 $pot = $db->super_query("SELECT a.*, WEEKDAY(a.begin) as beg, a.end as minutes FROM radioadmin a where a.wave='1' AND ((WEEKDAY(a.begin)='".$wday."'  AND TIME(a.begin)<='".$hday."'  AND TIME(a.end)>='".$hday."') OR ( (WEEKDAY(a.begin)='".$wday."' AND WEEKDAY(a.end)='".$wdaynext."'  AND TIME(a.begin)<='".$hday."'  )) OR  (WEEKDAY(a.end)='".$wday."'  AND WEEKDAY(a.begin)='".$wdayprev."'  AND TIME(a.end)>='".$hday."'))  order by id desc LIMIT 0,1;");
...
{
	  $db->query( "UPDATE radiotop set mvotes2=0, mvotes1=0, month='".$curmonth."' " );
}
...
if($_POST['addvote']&&$member_id[user_group]<=3)
{
	$votad=stripslashes($_POST['addvote']);
		$db->query( "INSERT INTO radiovote  (name) values ('$votad')" );
			echo("Добавлено!");
}
...
if($_GET['votename'])
{
	...
	$sname= ltrim(rtrim(str_replace ($vowels, " ",$_GET['sname'])));
	 $image = $db->super_query("SELECT * FROM radiovote WHERE wave='1' order by id desc  LIMIT 0,1;");
...
$_SESSION[voted]=$image[name];
	...
	$im = $db->super_query("SELECT  `vote1`, `vote2`, `vote3`, `name`    FROM radiovote where name!='no' AND wave='1'  order by id desc  LIMIT 0,1 ");
...
if($name=="" || !$name)
{
	$name="blogs";
	 ...
	 if($_GET['system']=="1")
	 {
	  $f3 = $db->query("SELECT  `vote1`, `vote2`, `vote3`, `name`    FROM radiovote where name!='no' AND wave='1'   order by id desc  LIMIT 0,1 ");
	 }
	 else
	 {
		 	  $f3 = $db->query("SELECT  `id`, `vote1`, `vote2`, `vote3`,  `name`   FROM radiovote   where name!='no' AND wave='1'  order by id desc  LIMIT 0,40 ");
	 }
	 $i1=1;
...
<input style="width: 500px;" type="text" name="addvote" id="addvote" placeholder="Как дела? 1. Хорошо 2. Плохо 3. Никак" />
...
{if($_GET[prid])
{
 $days=array("ПОНЕДЕЛЬНИК","ВТОРНИК","СРЕДА","ЧЕТВЕРГ","ПЯТНИЦА","СУББОТА","ВОСКРЕСЕНЬЕ");
  ...
  $wdayprev=6;
	...
	$pot = $db->super_query("SELECT a.*, b.foto,  WEEKDAY(a.begin) as beg, a.end as minutes FROM radioadmin a LEFT JOIN dle_users b  ON (a.dj = b.name)  where a.id='{$idp}'  order by id desc LIMIT 0,1;");
	 ...
	 $when=date("H:i", $time1);
	  ...
	  if($pot[dj])
	  {
		  $djs='<p><b>Автор:</b> <a onclick="DlePage(\'user='.$pot[dj].'\'); return false;" href="">'.$pot[dj].'</a></p>';
	  }
	  
	  $nam_e =stripslashes($pot[name]);
	  if($pot[description2])
	   $tpl->set( '{potok6}', $djs.stripslashes($pot[description2]));
	  else
				   $tpl->set( '{potok6}', $djs.stripslashes($pot[description]));
				   if($pot[link1])
				   {
				 $link1= '<a href="'.$pot[link1].'"><img  width="15" height="15" src="http://nocens.ru/pics2/mycens.png"  align="absmiddle"><b>Архив выпусков передачи</b></a> ';
				   }
				    $tpl->set( '{potok7}', $link1);
				if($pot[link2])
				   {
				 $link2= '<a href="'.$pot[link2].'"><img width="15" height="15" src="http://nocens.ru/pics3/interface/comments.png"  align="absmiddle"><b>Обсуждение передачи</b></a> ';
				   }
				   	if($pot[vk])
				   {
				 $link2.= '<a target="_blank" href="http://vk.com/'.$pot[vk].'"><img width="15" height="15" src="http://nocens.ru/pics3/interface/groupvk.png"  align="absmiddle"><b>Вконтакте</b></a> ';
				   }
				   	if($pot[twitter])
				   {
				 $link2.= '<a target="_blank"  href="http://twitter.com/'.$pot[twitter].'"><img width="15" height="15" src="http://nocens.ru/pics3/interface/grouptw.png"  align="absmiddle"><b>Twitter</b></a> ';
				   }
				   if($member_id[user_group]<4)
				   {
				...
				if($pot[call]&&$pot['efir']=="1")
				   {
				 $link10= '<p style="font-size: 16px; color: #36C; font-style: italic;"><b>Прямой эфир, принимаются Skype-звонки!</b></p>';
				   }
				   else if ($pot['efir']=="1")
				   {
...
$descr = str_replace("\n","<br>",$descr);
	...
	$TplThumb = str_replace("{author}", '<p style="color: #FFF;"><span><b>'.substr($im[h1],0,strlen($im[h1])-3).'</b></span></p>', $TplThumb);
...
else
{
  ...
  $wdayprev=6;
	...
	$pot = $db->super_query("SELECT *,  WEEKDAY(begin) as beg, end as minutes FROM radioadmin where wave='1' AND  (WEEKDAY(begin)='".$wday."'  AND TIME(begin)<='".$hday."'  AND TIME(end)>='".$hday."') OR ( (WEEKDAY(begin)='".$wday."' AND WEEKDAY(end)='".$wdaynext."'  AND TIME(begin)<='".$hday."'  )) OR  (WEEKDAY(end)='".$wday."'  AND WEEKDAY(begin)='".$wdayprev."'  AND TIME(end)>='".$hday."') order by id desc LIMIT 0,1;");
	
	 		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, TIME(begin) as h1 FROM radioadmin where wave='1'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
...
$pot = $db->super_query("SELECT *,  WEEKDAY(begin) as beg, end as minutes FROM radioadmin where wave='1' AND hidden!='1' AND  ((WEEKDAY(begin)='".$wday."'  AND TIME(begin)<='".$hday."'  AND TIME(end)>='".$hday."') OR ( (WEEKDAY(begin)='".$wday."' AND WEEKDAY(end)='".$wdaynext."'  AND TIME(begin)<='".$hday."'  )) OR  (WEEKDAY(end)='".$wday."'  AND WEEKDAY(begin)='".$wdayprev."'  AND TIME(end)>='".$hday."')) order by type asc, id desc LIMIT 0,1;");
		 ...
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, TIME(begin) as h1, TIME(end) as h2 FROM radioadmin where wave='1' AND hidden!='1'  AND   WEEKDAY(begin)='".$wday."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
	 ...
	 $k1=0; $vowels = array("\"");
		...
		if($im[name]==$pot[name])
			{	
	$progs.='<textformat leading="5">&nbsp;&nbsp;<font size="+2" color="#990000"> <a target="_blank" href="http://nocens.ru/index.php?do=radio2&schedule=1&prid='.$im[id].'"><b>'.substr($im[h1],0,strlen($im[h1])-3).' &nbsp;&nbsp;'.$im[name].'</b></a> (в эфире)</font></textformat><br>';
			$next1=1;		}
		else if($next1==1)
		{
			$progs.='<font size="+1"  color="#000000"> <a target="_blank" href="http://nocens.ru/index.php?do=radio2&schedule=1&prid='.$im[id].'"><b>'.substr($im[h1],0,strlen($im[h1])-3).'</b> &nbsp;&nbsp;'.$im[name].' </a></font><br>';
			
		}
		
	else
	{
		$progs.='<font color="#666666"> <a target="_blank" href="http://nocens.ru/index.php?do=radio2&schedule=1&prid='.$im[id].'"><b>'.substr($im[h1],0,strlen($im[h1])-3).'</b> &nbsp;&nbsp;'.$im[name].'</a></font><br>';		
			}
...
else if($_GET['todayxml'])
{
	   ...
	   if($wday=='-1')
	   $wday=6;
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where   wave='1' AND hidden!='1'  AND  WEEKDAY(begin)='".$wday."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if(isset($_GET['dayxml']))
{
		 ...
		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where wave='1' AND hidden!='1'  AND   WEEKDAY(begin)='".$_GET['dayxml']."'  order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if($_GET['weekxml'])
{
	 		 ...
	 		 $fsch = $db->query("SELECT *, WEEKDAY(begin) as beg, begin as h1, end as h2 FROM radioadmin where wave='1' AND hidden!='1'  AND  type='2' order by  WEEKDAY(begin) asc, TIME(begin) asc  LIMIT 0,200;");
		 $days=array("ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС");
...
else if($_GET['djbar']&&$member_id[user_group]<=3)
{
	...
	else if($_POST[potok]||$_POST[potok2])
	{
		...
		$typ=1;
			  if($_GET[prid])
			  {
		   $db->query( "UPDATE radioadmin set `src`='" . $potok . "', `src2`='" . $potok64 . "',`src3`='" . $potok256 . "', `efir`='".$efir."', `name`='" . $potok2 . "', `end`='".$month."', `begin`='".$month2."', `type`='".$typ."', `picture`='".$picl."',`link1`='".$link1."',`link2`='".$link2."',`description`='".$dsc."',`description2`='".$dsc2."', `call`='".$call."',`dj`='".$djay."',`overlay`='".$overlay."',`hidden`='".$hidden."', vk='".$vk."',twitter='".$twitter."'    where id = '".$_GET[prid]."'" );
			  }
			  else
			  {
		$db->query("DELETE FROM radioadmin WHERE type='1' && begin <= NOW()");
	
								  $db->query("INSERT  INTO radioadmin (`src`,`src2`,`src3`, `name`, `end`, `begin`, `type`, `picture`,`link1`,`link2`,`description`,`description2`,`call`,`efir`,`dj`,`overlay`,`hidden`,`vk`,`twitter`) values ('" . $potok . "','" . $potok64 . "','" . $potok256 . "','" . $potok2 . "', '".$month."', '".$month2."','".$typ."','".$picl."','".$link1."','".$link2."','".$dsc."','".$dsc2."','".$call."','".$efir."','".$djay."','".$overlay."','".$hidden."','".$vk."','".$twitter."')");
			  }
...
$pot = $db->super_query("SELECT *,  WEEKDAY(begin) as beg, end as minutes FROM radioadmin where wave='1' AND  (WEEKDAY(begin)='".$wday."'  AND TIME(begin)<='".$hday."'  AND TIME(end)>='".$hday."') OR ( (WEEKDAY(begin)='".$wday."' AND WEEKDAY(end)='".$wdaynext."'  AND TIME(begin)<='".$hday."'  )) OR  (WEEKDAY(end)='".$wday."'  AND WEEKDAY(begin)='".$wdayprev."'  AND TIME(end)>='".$hday."') order by id desc LIMIT 0,1;");
   $time1= time();
	 ...
	 $k1=0;
		...
		$progs.=$alert.$delb.$pl."[".substr($im[h1],0,strlen($im[h1])-3)."] <a ".$hidder." href=\"http://nocens.ru/index.php?do=radio2&djbar=1&prid=".$im[id]."\">".$im[name].$pm."</a><br>";
...
if($name=="" || !$name)
{
	$name="blogs";
	 ...
	 if($_GET['system']=="1")
	 {
	  $f3 = $db->query("SELECT  `id`,`songname`, `votes`, `votes2`, (votes-votes2) as vote3 FROM radiotop WHERE wave='1'   order by vote3 desc, votes desc  LIMIT 0,10 ");
	 }
	 else
	 {
		 	  $f3 = $db->query("SELECT  a.id,a.songname, a.mvotes1, a.mvotes2, a.votes, a.votes2, (a.votes-a.votes2) as vote3,(a.mvotes1-a.mvotes2) as mvotes, a.name,b.foto  FROM radiotop a LEFT JOIN dle_users b ON (a.name = b.name)  WHERE a.wave='1'  order by mvotes desc, vote3 desc, a.votes desc  LIMIT 0,50 ");
	 }
	    while ($im = $db->get_row($images)){
...
$col="#FCCA45";
		...
		$sizer=28;
					...
					if($im[foto])
		{
			  $imginfo = @getimagesize("http://nocens.ru/uploads/fotos/" . $im[foto]);
					if($imginfo[1]==0){ $imginfo[1]=100; $imginfo[0]=100;}

					$img_koeff = $imginfo[1]/$imginfo[0];
					$imgal=intval(45*$img_koeff-15);
                    $imgw=$img_koeff*45;
		$foto='<div class="ava2"><a title="'.$im[name].'" href="http://nocens.ru/index.php?user='.$im[name].'"><img  width="45"  height="'.$imgw.'"  src="http://nocens.ru/uploads/fotos/'.$im[foto].'"></a></div>';
		}
		else if ($im[name])
		{
			$foto='<div class="ava2"><a title="'.$im[name].'" href="http://nocens.ru/index.php?user='.$im[name].'"><img width="45"  height="45"  src="http://nocens.ru/uploads/fotos/que.png"></a></div>';
		}
		else
		{
			$foto='<div class="ava2"><img title="Автор не указан" width="45"  height="45"  src="http://nocens.ru/uploads/fotos/que.png"></div>';
		}
		if($a1[$i1])
		{
			
			$shout.= '<tr><td colspan="4"><div class="infoinside2" style="padding-top: 10px;">'.$a1[$i1].'</div></td></tr><tr style=" background-color: #CCC; text-shadow: 0px 1px 0px rgba(255, 255, 255, 0.5);"><td width="80" class="pm_head"></td><td width="600" class="pm_head">Название передачи</td><td class="pm_head" align="center">Рейтинг (месяц)</td><td class="pm_head" align="center">Рейтинг (всего)</td></tr>';
		}
		
		if($member_id['user_group']=='1'||$member_id['user_group']=='3')
			{
				
				$modr=' <a target="_blank" href="http://nocens.ru/index.php?do=radio&del='.$im[id].'">[X]</a>';
			}
			 if($_GET['system']!="1")
 {
//$shout.=$foto.' <span style="font-size: '.$sizer.'px; color: '.$col.'; ">'.$i1.'. <b>'.$im[songname].'</span></b> <sup> <b><font color="#DDFEDA">+'.$im[votes].'</font> <font color="#CC0000">'.$im[votes2].'</font></b></sup>'.$modr.'<div  style="clear: both;"></div>';
			$shout.='<tr><td class="pm_list">'.$foto.'</td><td class="pm_list" align="left"><h2>'.$i1.'. '.$im[songname].'</h2><a href="http://nocens.ru/index.php?user='.$im[name].'">'.$im[name].'</a> '.$modr.'<br></td><td class="pm_list"  align="center"><font color="green"><b>'.$im[mvotes].'</b></font></td><td class="pm_list" align="center"><font color="green">'.$im[vote3].'</font></td></tr>';
 }
 else
 {
				if(strlen( $im[songname]) > 30 ) 
...
$shout= ''.mb_convert_encoding('<br>'.$shout,"UTF8", "CP1251");
 ...
 else
 {
	 ...
	 $i2=-1;
	  $i1=1;
	   $f3 = $db->query("SELECT  `id`, `songname`, `votes2` FROM radiotop WHERE wave='1'  order by votes2 desc  LIMIT 0,50 ");
...
if($_GET['type']!="dj")
{
		    $f3 = $db->query("SELECT `id`, `autor`, `date`, `short_story`,`type`,`autorvk` FROM radiomes WHERE wave='1'  order by id desc  LIMIT ".$from.",40 ");
}
else
{
	 $f3 = $db->query("SELECT  id, autor, short_story, date, type, autorvk FROM radiomes WHERE wave='1' order by id desc  LIMIT ".$from.",40 ");
}
			$i=39;
